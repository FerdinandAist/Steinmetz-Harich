...
function Hero({ page, quiet = false }) {
  return (
    <section className={'hero' + (quiet ? ' hero--quiet' : '')}>
...
function TrustStrip() {
  return (
    <section className="trust-band" aria-label="Vertrauenssignale">
...
function ProcessSection() {
  ...
  return <section className="section"><div className="container split"><div><p className="eyebrow">Vom Gespräch zum Stein</p><h2>Ein klarer Ablauf nimmt Unsicherheit.</h2><p>Gerade in einer schweren Situation hilft ein ruhiger Prozess. Schritt für Schritt wird sichtbar, welche Gestaltung dem Menschen und dem Ort gerecht wird.</p></div><div className="steps">{steps.map(([num, title, text]) => <article key={num}><span>{num}</span><h3>{title}</h3><p>{text}</p></article>)}</div></div></section>;
}
...
function FaqSection({ faq = [] }) {
  ...
  return <section className="section faq-section"><div className="container section-head"><p className="eyebrow">Fragen</p><h2>Was vor dem ersten Gespräch oft wichtig ist</h2></div><div className="container faq-grid">{faq.map(([question, answer]) => <details key={question}><summary>{question}</summary><p>{answer}</p></details>)}</div></section>;
}

function ContactBand() {
  return <section className="section contact-band" id="kontaktformular"><div className="container contact-panel contact-panel--wide"><div><p className="eyebrow">Nächster Schritt</p><h2>Beratung für Ihr Steinwerk anfragen</h2><p>Ein Anruf oder eine kurze Nachricht genügt. Das erste Gespräch darf offen beginnen - auch wenn Form, Stein oder Schrift noch unklar sind.</p><div className="contact-actions"><a href={company.phoneHref}><strong>{company.phone}</strong><span>Direkt anrufen</span></a><a href={company.emailHref}><strong>{company.email}</strong><span>E-Mail schreiben</span></a></div></div><ContactForm source="kontaktbereich" /></div></section>;
}
